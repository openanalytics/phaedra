...
public class CurveKindQueryValuePanelFactory extends EnumeratedTextQueryValuePanelFactory {
	private static Set<QueryFilter> FILTERS = Collections.unmodifiableSet(new HashSet<>(Arrays.asList(
			new QueryFilter(CRCurve.class, "kind", OperatorType.STRING, Operator.STRING_EQUALS, null))));

	private static List<String> CURVE_KINDS = CollectionUtils.transform(CurveKind.values(), c -> c.toString());
