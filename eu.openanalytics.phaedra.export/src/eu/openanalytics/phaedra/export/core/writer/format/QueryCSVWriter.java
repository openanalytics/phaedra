...
public class QueryCSVWriter extends AbstractCSVWriter implements IExportWriter {
	...
	public void finish() throws IOException {
		try {
			...
			try (au.com.bytecode.opencsv.CSVWriter writer = new au.com.bytecode.opencsv.CSVWriter(
					new FileWriter(getDestinationPath(getSettings().getDestinationPath(), "Info")), columnSeparator, quoteChar, escapeChar)) {
	...
	private void writeMainTable() throws IOException {
		try (au.com.bytecode.opencsv.CSVWriter writer = new au.com.bytecode.opencsv.CSVWriter(new FileWriter(getSettings().getDestinationPath()),
				columnSeparator, quoteChar, escapeChar)) {
			int colCount = baseResult.getColumnCount() + featureResults.stream().mapToInt(qr -> qr.getColumnCount()).sum();
			int colIndex = 0;
			String[] rowData = new String[colCount];
			for (int c = 0; c < baseResult.getColumnCount(); c++) rowData[colIndex++] = baseResult.getColumn(c).getName();
			for (QueryResult res: featureResults) {
				for (int c = 0; c < res.getColumnCount(); c++) rowData[colIndex++] = res.getColumn(c).getName();
			}
			writer.writeNext(rowData);
			
			for (int r = 0; r < baseResult.getRowCount(); r++) {
				colIndex = 0;
				for (int c = 0; c < baseResult.getColumnCount(); c++) {
					boolean numeric = baseResult.isColumnNumeric(c);
					if (numeric) {
						if (baseResult.getColumn(c).getName().endsWith("_ID")) rowData[colIndex++] = "" + Double.valueOf(baseResult.getNumericValue(r, c)).longValue();
