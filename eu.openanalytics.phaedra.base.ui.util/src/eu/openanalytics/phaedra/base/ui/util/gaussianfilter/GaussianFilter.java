...
public class GaussianFilter extends ConvolveFilter {
	...
	public ImageData filter(ImageData src, ImageData dst) {
		...
		if (dst == null) {
			dst = new ImageData(src.width, src.height, src.depth, src.palette);
		}
		...
		for (int i = 0; i < inPixels.length; i++) {
			RGB rgb = src.palette.getRGB(inPixels[i]);
			inPixels[i] = rgb.blue + (rgb.green << 8) + (rgb.red << 16);
		}
		...
		for (int i = 0; i < inPixels.length; i++) {
			...
			int g = (argb >> 8) & 0xFF;
			int b = argb & 0xFF;
			inPixels[i] = dst.palette.getPixel(new RGB(r, g, b));
		}
		
		dst.setPixels(0, 0, width*height, inPixels, 0);
		return dst;
	}
