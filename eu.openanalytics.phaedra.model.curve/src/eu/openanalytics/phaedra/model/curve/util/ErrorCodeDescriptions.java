...
public class ErrorCodeDescriptions {
	...
	public static String getErrorDescription(Curve curve) {
		if (curve.getFitError() == 0) return "No problems or irregularities found.";
		if (curve instanceof OSBCurve) return getReceptorErrorDescription(curve.getFitError());
		return null;
	}
	
	public static String getReceptorErrorDescription(int code) {
		// Note: these descriptions are taken from the document receptor-vignette.pdf in receptor_0.0-31
		switch (code) {
		case -3: return "Fit Error -3:\n\n"
