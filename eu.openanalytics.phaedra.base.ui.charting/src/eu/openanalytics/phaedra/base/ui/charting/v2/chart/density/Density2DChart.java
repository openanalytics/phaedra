...
public class Density2DChart<ENTITY, ITEM> extends AbstractChart<ENTITY, ITEM> {
	...
	public TablePlot createPlot() {
		...
		return new DensityPlot(plotSurface);
	}
	...
	public void updatePlotData() {
		...
		PtPlotSurface surface = (PtPlotSurface)((DensityPlot)getPlot()).getSurface();
	...
	public void settingsChanged() {
		...
		state.setGaussianFilter(settings.getGauss());
	...
	public void setSelection(BitSet bitSet) {
		if (bitSet != null) {
			((DensityPlot) getPlot()).setSelectionBitSet(bitSet);
		}
	}

	@Override
	public BitSet getActiveSelection() {
		return ((DensityPlot) getPlot()).getSelectionBitSet_();
	}
	...
	public BitSet getSelection(Shape shape, boolean isSingleSelection) {
		if (isSingleSelection) {
			...
			int index = ((DensityPlot) getPlot()).getPlottedPointIterator().getClosestPoint(point, 10);
			if (index >= 0) {
				selection.set(index);
			}
			return selection;
		}
		return ((DensityPlot) getPlot()).getPlottedPointIterator().getContainedPoints(shape);
	}
