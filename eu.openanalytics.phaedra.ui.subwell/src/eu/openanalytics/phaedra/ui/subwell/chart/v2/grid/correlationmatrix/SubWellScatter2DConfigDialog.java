...
public class SubWellScatter2DConfigDialog extends SubWellChartConfigDialog {
	...
	public SubWellScatter2DConfigDialog(Shell parentShell, SubWellScatter2DLayer layer) {
		...
		for (IFilter<Well, Well> filter : layer.getDataProvider().getFilters()) {
			if (filter instanceof MinMaxFilter) {
				MinMaxFilter<Well, Well> minMaxFilter = (MinMaxFilter<Well, Well>) filter;
	...
	protected void createAuxiliaryTab(List<String> featureNames, TabFolder tabFolder) {
		...
		for (int i = 0; i < currentAuxSettings.length; i++) {
			final int index = i;
	...
	protected void createGroupingTab(TabFolder tabFolder) {
		...
		int select = 0;
		int index = 0;
		for (IGroupingStrategy<Well, Well> strat : groupingStrategies) {
	...
	protected void createFilterTab(TabFolder tabFolder) {
		...
		for (final IFilter<Well, Well> filter : getLayer().getDataProvider().getFilters()) {
	...
	private void createFilterButton(Composite filterComp, final IFilter<Well, Well> filter, final String filterItem) {
	...
	protected void cancelPressed() {
		...
		for (IFilter<Well, Well> filter : getLayer().getDataProvider().getFilters()) {
			if (filter instanceof MinMaxFilter) {
				filter = new MinMaxFilter<Well, Well>(getLayer().getDimensionCount(), getLayer().getDataProvider());
				((MinMaxFilter<Well, Well>) filter).setActiveFilterItems(minMaxFiltersActiveItemsValue);
				((MinMaxFilter<Well, Well>) filter).setMin(minMaxFiltersValue[0]);
				if (minMaxFiltersValue.length > 1) ((MinMaxFilter<Well, Well>) filter).setMax(minMaxFiltersValue[1]);
			}
		}
