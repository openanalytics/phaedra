...
public class PlateUploadStatusQueryValuePanelFactory extends EnumeratedIntegerQueryValuePanelFactory {
	private static Set<QueryFilter> FILTERS = Collections.unmodifiableSet(new HashSet<>(Arrays.asList(
			new QueryFilter(Plate.class, "uploadStatus", OperatorType.NATURAL_NUMERIC, Operator.EQUALS, null))));
	
	private static List<Integer> UPLOAD_STATUSES = Lists.transform(Arrays.asList(PlateUploadStatus.values()), new Function<PlateUploadStatus, Integer>() {
		public Integer apply(PlateUploadStatus plateUploadStatus) {
	...
	public LabelProvider getLabelProvider() {
		return new LabelProvider() {
			...
			public String getText(Object element) {
				String upperCase = PlateUploadStatus.getByCode((Integer) element).toString();
